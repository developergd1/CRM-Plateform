      ...
      <div className="max-w-md w-full mx-auto my-auto py-8 z-10 flex flex-col items-center">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-xs text-slate-400 mt-1 max-w-sm">
            Sign in with your Client ID (CLI-XXXXX), Employee ID, or Registered Email
          </p>
        </div>

        {/* Global Error / Admin Redirect Banner */}
        {errorMsg && (
          <div className="w-full mb-5 p-4 bg-rose-950/90 border-2 border-rose-500/80 text-rose-200 text-xs rounded-2xl space-y-2 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            {isAdminNotice && (
              <div className="pt-2 border-t border-rose-800/80">
                <Link
                  href="/growthIndia"
                  className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-growth-gold to-amber-600 text-slate-950 font-black text-xs rounded-xl shadow-md hover:brightness-110 transition-all"
                >
                  <ShieldCheck className="w-4 h-4" />
                  <span>Open Secure Admin Console (/growthIndia)</span>
                </Link>
              </div>
            )}
          </div>
        ...
        <div className="w-full bg-slate-900/90 backdrop-blur-2xl border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. CLI-00001 or priya.patel@growthindia.in"
                  value={identifier}
          ...
          {/* Direct link to /growthIndia */}
          <div className="mt-6 pt-5 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
            <span>Platform Administrator?</span>
            <Link
              href="/growthIndia"
              className="inline-flex items-center gap-1 font-bold text-growth-gold hover:text-amber-300 hover:underline transition-all"
            >
              <ShieldCheck className="w-3.5 h-3.5 text-growth-gold" />
              <span>Admin Gateway (/growthIndia)</span>
            </Link>
          </div>
        </div>
      </div>
